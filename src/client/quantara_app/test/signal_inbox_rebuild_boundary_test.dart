...
void main() {
  ...
  test(
    'manual trade entry point is not deadlocked by stale account UI state',
    () {
      final source = File(
        'lib/features/owner_alpha/presentation/owner_alpha_signals.dart',
      ).readAsStringSync();
      ...
      expect(
        source,
        contains('private account snapshot'),
        reason:
            'The UI gate should document that account truth is refreshed by preflight.',
      );
    },
  );
