...
void main() {
  test('shadow evidence keeps actionable signal outcomes inside the experiment window', () {
    ...
    final evidence = buildTradingLabShadowEvidence(
      run,
      [inside, before, otherSymbol],
    );
  ...
  test('AI review remains useful when paper trades are zero but shadow outcomes exist', () {
    final startedAt = DateTime.utc(2026, 8, 10);
    final run = TradingLabRun(
      manifest: TradingLabRunManifest(
    ...
    final json = buildTradingLabAiReviewJsonWithShadows(
      run,
      [
        _entry(
          setupId: 'shadow-only',
