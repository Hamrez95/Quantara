...
void main() {
  ...
  test('uses explicit demo data after a bounded timeout', () async {
    final primary = ApiCockpitRepository(
  ...
  test('direct API repository construction rejects unsafe endpoints', () {
    expect(
      () => ApiCockpitRepository(
        client: MockClient((_) async => http.Response('', 500)),
        endpoint: Uri.parse('http://api.example.com/api/v1/cockpit'),
      ),
      throwsA(isA<CockpitContractException>()),
    );
  });

  test('rejects API origins with credentials, paths, or query strings', () {
    for (final value in [
      'https://user:secret@api.example.com',
      'https://api.example.com/v1',
      'https://api.example.com?mode=unsafe',
    ]) {
      expect(
        () => CockpitRepositoryFactory.create(
          apiBaseUrl: value,
