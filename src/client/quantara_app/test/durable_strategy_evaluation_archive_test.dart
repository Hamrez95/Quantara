...
void main() {
  ...
  test('bounded local retention keeps only the newest immutable runs', () async {
    final memory = _MemoryStore();
    final archive = DurableStrategyEvaluationArchive(
    ...
    expect(records.map((record) => record.runId), <String>['newest', 'middle']);
    expect(await archive.find('oldest'), isNull);
    expect(records.every((record) => !record.grantsLocalLiveAuthority), isTrue);
  });
