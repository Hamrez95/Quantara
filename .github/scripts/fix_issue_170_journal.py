...
""",
    ...
    return journalTradeId == other.journalTradeId &&
""",
    'idempotent exchange-close observation replay',
)
...
""",
    ...
    final close = authoritativeClosed.isNotEmpty
        ...
        : closed.last;
""",
    'authoritative close precedence over pending observation',
)

(ROOT / 'test/issue_170_pending_close_reconciliation_test.dart').write_text(r'''import 'package:flutter_test/flutter_test.dart';
...
void main() {
  test('repeated exchange-closed observation is idempotent across restart time', () {
    final initial = TradingJournalLedger.empty()
        .appendPlan(_plan())
        .appendEvent(_entry());
    final first = initial.appendEvent(
      _pendingClose(DateTime.utc(2026, 8, 7, 15, 30), historyAvailable: false),
    );
  ...
  test('verified older stop fill supersedes later pending close observation', () {
