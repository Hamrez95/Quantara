...
Path(
    "src/client/quantara_app/lib/features/owner_alpha/data/signal_sizing_reconciler.dart"
).write_text(
...
abstract final class SignalSizingReconciler {
  static SignalJournalEntry merge({
    ...
    final fresh = SignalJournalEntry.fromIdea(idea, sizingCapital: sizingCapital);
...
replace(
    controller,
    "        generatedAt: current.generatedAt,\n      );\n    }\n    notifyListeners();\n  }\n\n  static String? _normalizeSymbol",
...
background = "src/client/quantara_app/lib/features/owner_alpha/data/background_opportunity_scanner.dart"
replace(
    background,
...
source_test = Path("src/client/quantara_app/test/signal_leverage_ui_source_test.dart")
source_test.write_text(
    source_test.read_text().replace(
...
Path("src/client/quantara_app/test/signal_sizing_reconciler_test.dart").write_text(
...
void main() {
  ...
  test('active setup keeps its original historical sizing', () {
    final active = SignalJournalEntry.fromIdea(
      _idea(maximumLoss: 100, positionSize: 10, notionalValue: 10000),
      sizingCapital: 10000,
    ).copyWith(
...
Path("src/client/quantara_app/test/video_regression_source_test.dart").write_text(
    """import 'dart:io';

import 'package:flutter_test/flutter_test.dart';

void main() {
  test('video regressions stay wired to bounded leverage and sizing refresh', () {
    final signals = File(
    ...
    expect(controller, contains('await _captureOpportunities(_snapshot!.opportunities)'));
...
replace("src/client/quantara_app/pubspec.yaml", "version: 0.11.1+13", "version: 0.11.2+14")
workflow_path = Path(".github/workflows/flutter-ci.yml")
workflow = workflow_path.read_text().replace("0.11.1", "0.11.2")
workflow = workflow.replace("permissions:\n  contents: write", "permissions:\n  contents: read")
workflow = workflow.replace(
    "      - name: Checkout\n        uses: actions/checkout@v4\n        with:\n          ref: ${{ github.event.pull_request.head.ref }}\n          fetch-depth: 0\n",
    "      - name: Checkout\n        uses: actions/checkout@v4\n",
)
start = workflow.index("      # BEGIN ONE-TIME VIDEO PATCH\n")
end = workflow.index("      # END ONE-TIME VIDEO PATCH\n") + len(
    "      # END ONE-TIME VIDEO PATCH\n"
)
workflow_path.write_text(workflow[:start] + workflow[end:])

release = Path("docs/releases/v0.11.2-video-regressions.md")
...
Path(".github/workflows/apply-video-regression-patch.yml").unlink(missing_ok=True)
Path(".github/scripts/apply_video_regression_patch.py").unlink(missing_ok=True)
