...
if 'static const int maximumManualLeverage = 125;' not in text:
    text = text.replace(
...
replace_required(
    ...
    'TradeIdea.marginAt block',
)

controller = ROOT / 'lib/features/owner_alpha/application/owner_alpha_controller.dart'
replace_required(
    controller,
...
replace_required(
    ...
    'signal leverage dropdown',
)
text = signals.read_text()
if 'class _LeverageControl extends StatefulWidget' not in text:
    text += r'''
...
class _LeverageControlState extends State<_LeverageControl> {
  ...
  Widget build(BuildContext context) {
    final selected = _draft.round().clamp(
      1,
    ...
    final presets = <int>{
      ...
      widget.safeCap,
      selected,
    }.where((value) => value >= 1 && value <= TradeIdea.maximumManualLeverage)
...
analysis = ROOT / 'lib/features/owner_alpha/presentation/owner_alpha_analysis.dart'
replace_required(
    analysis,
    '''              Wrap(
                spacing: 8,
...
replace_required(
    ...
    'analysis leverage dropdown',
)

pubspec = ROOT / 'pubspec.yaml'
text = pubspec.read_text()
if 'version: 0.11.0+12' in text:
    pubspec.write_text(text.replace('version: 0.11.0+12', 'version: 0.11.1+13'))

(ROOT / 'test/leverage_selection_test.dart').write_text(r'''import 'package:flutter_test/flutter_test.dart';
...
(ROOT / 'test/signal_leverage_ui_source_test.dart').write_text(r'''import 'dart:io';

import 'package:flutter_test/flutter_test.dart';

void main() {
  test('signal leverage uses bounded inline controls instead of dropdown menus', () {
    final signals = File(
    ...
    expect(signals, contains('class _LeverageControl extends StatefulWidget'));
