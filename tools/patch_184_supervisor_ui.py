...
if needle not in auto and "_buildSupervisorSupportSessionCard()," not in auto:
    raise SystemExit("Supervisor UI insertion anchor not found")
if "_buildSupervisorSupportSessionCard()," not in auto:
    auto = auto.replace(needle, replacement, 1)
AUTO_PATH.write_text(auto)

tools = TOOLS_PATH.read_text()
function_anchor = "  Future<void> _exportLocalLiveDiagnostics(\n"
function_pos = tools.find(function_anchor)
if function_pos < 0:
    raise SystemExit("Diagnostic export function not found")
start_marker = "      final snapshot = widget.accountController.snapshot;\n"
start = tools.find(start_marker, function_pos)
end_marker = "      final stamp = generatedAt.toIso8601String().replaceAll(\n"
end = tools.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit("Diagnostic export section anchors not found")
export_replacement = """      final sections = await _buildLocalLiveDiagnosticSections(events);
      final json = LocalLiveDiagnosticBundle.encode(
        generatedAt: generatedAt,
        sections: sections,
      );
"""
if "_buildLocalLiveDiagnosticSections(events);" not in tools[function_pos:end]:
    tools = tools[:start] + export_replacement + tools[end:]

marker = "// AI Supervisor support-session transport — Issue #184."
...
extension _LocalLiveSupervisorSupportTools on _LocalLiveTradeControlCardState {
  ...
  Widget _buildSupervisorSupportSessionCard() {
    ...
    final active = local != null &&
        remote != null &&
        now.isBefore(remote.expiresAtUtc);
            ...
            if (state.error != null) ...[
              ...
              Text(
                ...
                style: Theme.of(context)
                    .textTheme
                    .bodySmall
                    ?.copyWith(color: QuantaraColors.danger),
              ),
            ],
            const SizedBox(height: 12),
            Wrap(
              ...
              children: active
                  ? [
                      ...
                      OutlinedButton.icon(
                        ...
                        label: Text(_t('کپی توکن موقت', 'Copy temporary token')),
                      ),
                      TextButton.icon(
                        onPressed: state.busy
                            ? null
  ...
  Future<void> _enableSupervisorSupportSession() async {
    ...
    try {
      final serverConfig = await const SecureAutoTradeServerConfigStore().load();
  ...
  Future<Map<String, Object?>> _buildLocalLiveDiagnosticSections(
    ...
    return <String, Object?>{
      ...
      'privateAccountReconciliation': <String, Object?>{
        ...
        'lastAttemptAt': reconciliation.lastAttemptAt?.toUtc().toIso8601String(),
      ...
      'auditEvents': events.map((item) => item.toJson()).toList(growable: false),
